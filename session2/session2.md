# Session 2: RNA-Seq data processing and analysis
...
- [Session 2.1: Genomic Alignments Class Material](#session-41-genomic-alignments-class-material)
...
- [Session 2.2: Transcriptome Quantification class material](#session-42-transcriptome-quantification-class-material)
- [Exercise 3: Quantify with the RSEM program](#exercise-3-quantify-with-the-rsem-program)
- [Session 2.3: sequencing data visualization class material](#session-43-sequencing-data-visualization-class-material)
...
## Session 2.1: Genomic Alignments Class Material
...
```
$ ssh your_user@hpc.umassmed.edu
```
...
```
$ /pi/alper.kucukural-umw/umw_biocore/dnext_data/bin/qlogin
```

- Tip: If you want to learn the parameters of `qlogin` script, simply run `cat /pi/alper.kucukural-umw/umw_biocore/dnext_data/bin/qlogin`
...
```
$ /pi/alper.kucukural-umw/umw_biocore/dnext_data/bin/session2.sh
```
...
4. Please do it for the other samples **(control_rep2, control_rep3, exper_rep1, exper_rep2, exper_rep3)**.
   ...
   (`/pi/alper.kucukural-umw/umw_biocore/class/starruns.sh`).
   You can check what we have in this file using more/less or vi commands.

# Wait here for the intro lecture on quantification

## Session 2.2: Transcriptome Quantification class material

You can follow the class materials below.

<b>Session 2.2: Transcriptome Quantification</b><br />
...
$ cd ~/bootcamp/RNA-Seq
$ mkdir rsem
$ rsem-calculate-expression --star --star-output-genome-bam --paired-end -p 2 reads/control_rep1.1.fq reads/control_rep1.2.fq mm10/rsem/mm10.rsem rsem/ctrl1.rsem
```
...
rsem/ctrl1.rsem.STAR.genome.bam
...
```
$ rsem-calculate-expression --star --star-output-genome-bam --paired-end -p 2 reads/control_rep2.1.fq reads/control_rep2.2.fq mm10/rsem/mm10.rsem rsem/ctrl2.rsem
```
...
```
$ /pi/alper.kucukural-umw/umw_biocore/class/rsemruns.sh
```
...
## Session 2.3: sequencing data visualization class material

You can follow the class materials below.

<b>Session 2.3: Sequencing data visualization</b><br />
...
Transfer your bam files from the hpcc to your laptop using **FileZilla** (or use `scp` command in mac). If you haven't installed it, please check [our FileZilla guide in session 1](https://github.com/UMMS-Biocore/bootcamp/blob/master/session1/session1.md#file-transfer-from-your-laptop-to-cluster-using-FileZilla)
...
$ mkdir results
$ module load rsem/1.3.3
$ rsem-run-ebseq quant/rsem.gene.summary.count.txt 3,3 results/de.txt
$ rsem-control-fdr results/de.txt 0.01 results/fdr.de.txt
